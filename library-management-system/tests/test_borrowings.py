...
def available_book(db):
    book = Book(
        title="Test Book", author="Author", isbn="999-9-99-999999-9",
...
def test_borrow_default_due_date(client, user_headers, available_book):
    response = client.post("/borrowings/", json={"book_id": available_book.id}, headers=user_headers)
    assert response.status_code == 201
...
def test_return_book(client, user_headers, available_book):
    ...
    borrowing = client.post(
        "/borrowings/", json={"book_id": available_book.id, "due_date": due}, headers=user_headers,
    ).json()
